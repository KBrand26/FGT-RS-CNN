...
def train_merged(x_train, y_train, aux_y_train, x_val, y_val, aux_y_val, x_test, y_test, aux_y_test, main_weight, aux_weight, run):
    ...
    #Tensorboard setup
    run_logdir = os.path.join(os.curdir, f"../../lr_logs/merged_{main_weight}_{aux_weight}_run{run}")
    
    # Create utility callbacks
    early_stop = keras.callbacks.EarlyStopping(patience=5)
    save = keras.callbacks.ModelCheckpoint(f"../../models/merged_{main_weight}_{aux_weight}_model{run}.h5", save_best_only=True)
    tensorboard = keras.callbacks.TensorBoard(run_logdir)
    ...
    merged_cnn = keras.models.load_model(f"../../models/merged_{main_weight}_{aux_weight}_model{run}.h5")
    return merged_cnn.evaluate(x_test, test_labels)

def construct_merged(main_weight, aux_weight):
    """Construct the merged network.
    Parameters
    ----------
    main_weight : float
        The weight of the main output in the network.
    aux_weight : float
        The weight of the auxiliary output in the network
    ...
    # Second dense layer
    dense1 = keras.layers.Dense(71, activation='elu', kernel_initializer="he_normal")(drop0)
    #Auxiliary Outputs
    bent_out = keras.layers.Dense(units=1, activation='sigmoid', name="bent_out")(drop0)
    fr_out = keras.layers.Dense(units=1, activation='sigmoid', name="fr_out")(drop0)
    cores_out = keras.layers.Dense(units=1, activation='sigmoid', name="cores_out")(drop0)
    size_out = keras.layers.Dense(units=1, activation='sigmoid', name="size_out")(drop0)
    concat = keras.layers.concatenate([dense1, bent_out, fr_out, cores_out, size_out])
    drop1 = keras.layers.Dropout(0.5)(concat)

    #Output
    main_out = keras.layers.Dense(units=4, activation='softmax', name="main_out")(drop1)

    merged = keras.Model(inputs=[cnn_input], outputs=[main_out, bent_out, fr_out, cores_out, size_out])
...
def train_ultihead_man(x_train, y_train, aux_y_train, x_val, y_val, aux_y_val, x_test, y_test, aux_y_test, main_weight, aux_weight, run):
    ...
    #Tensorboard setup
    run_logdir = os.path.join(os.curdir, f"../../lr_logs/man_multihead_{main_weight}_{aux_weight}_run{run}")
    
    # Create utility callbacks
    early_stop = keras.callbacks.EarlyStopping(patience=5)
    save = keras.callbacks.ModelCheckpoint(f"../../models/man_multihead_{main_weight}_{aux_weight}_model{run}.h5", save_best_only=True)
    tensorboard = keras.callbacks.TensorBoard(run_logdir)
    ...
    multihead_cnn = keras.models.load_model(f"../../models/man_multihead_{main_weight}_{aux_weight}_model{run}.h5")
    return multihead_cnn.evaluate(x_test, test_labels)
